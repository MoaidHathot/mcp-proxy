...
/// </summary>
public sealed class WhitelistFilter : IToolFilter
{
    ...
    /// <summary>
    /// Initializes a new instance of <see cref="WhitelistFilter"/>.
    /// </summary>
    ...
    public WhitelistFilter(IEnumerable<string> patterns, bool caseInsensitive = true)
    {
...
/// </summary>
public sealed class BlacklistFilter : IToolFilter
{
    private readonly WhitelistFilter _innerFilter;

    /// <summary>
    /// Initializes a new instance of <see cref="BlacklistFilter"/>.
    /// </summary>
    /// <param name="patterns">Patterns to exclude (supports * and ? wildcards).</param>
    /// <param name="caseInsensitive">Whether matching is case-insensitive.</param>
    public BlacklistFilter(IEnumerable<string> patterns, bool caseInsensitive = true)
    {
        _innerFilter = new WhitelistFilter(patterns, caseInsensitive);
    }

    /// <inheritdoc />
    public bool ShouldInclude(Tool tool, string serverName)
    {
        // Include if NOT matched by the blacklist
...
public static class FilterFactory
{
    ...
    public static IToolFilter Create(FilterConfiguration config)
    {
        ...
        {
            FilterMode.Whitelist => new WhitelistFilter(config.Patterns, config.CaseInsensitive),
            FilterMode.Blacklist => new BlacklistFilter(config.Patterns, config.CaseInsensitive),
